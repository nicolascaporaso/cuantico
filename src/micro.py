...
def _leer_raw(samples_16k):
    ...
    with _stream_lock:
        if _micro_suspended or _stream is None:
            raise MicrofonoSuspendido(_micro_suspend_reason or "micro-suspendido")
...
def inicializar():
    global _pa, _stream, _oww, _vad, _input_device_index, _micro_suspended, _micro_suspend_reason
...
def suspender(reason: str = "music-playback") -> dict:
    global _micro_suspended, _micro_suspend_reason
    ya_suspendido = _micro_suspended
    _micro_suspended = True
    _micro_suspend_reason = reason
    with _stream_lock:
        _cerrar_stream()
    estado = {"suspended": True, "already_suspended": ya_suspendido, "reason": reason}
...
def reanudar() -> dict:
    ...
    with _stream_lock:
        ...
        _micro_suspended = False
        _micro_suspend_reason = ""
    estado = {"suspended": False, "stream_reopened": reabierto}
...
def escuchar():
    ...
    try:
        ...
        _debug_emit("listen-radar-suspended", {"reason": _micro_suspend_reason or "music-playback"})
        raise
...
def escuchar_seguimiento(timeout_ms=8000):
    ...
    try:
        ...
        _debug_emit("followup-suspended", {"timeout_ms": timeout_ms, "reason": _micro_suspend_reason or "music-playback"})
        raise


def cerrar():
    global _stream, _pa, _micro_suspended, _micro_suspend_reason
    with _stream_lock:
        _cerrar_stream()
    if _pa:
        try:
